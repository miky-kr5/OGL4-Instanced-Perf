...
void Obj::readVertex(std::string line, Mesh* mesh, bool &first)
{
	...
	else if (line[1] != 'p' && line[1] != 'c')
	{
		...
		{
			MinCoord.x = std::fmin(MinCoord.x, x); MinCoord.y = std::fmin(MinCoord.y, y); MinCoord.z = std::fmin(MinCoord.z, z);
			MaxCoord.x = std::fmax(MaxCoord.x, x); MaxCoord.y = std::fmax(MaxCoord.y, y); MaxCoord.z = std::fmax(MaxCoord.z, z);
		}
